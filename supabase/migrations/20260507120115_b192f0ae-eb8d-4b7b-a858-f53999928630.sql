SELECT set_config('request.jwt.claims','{"sub":"cdd5a5ad-cb74-45fa-9deb-79faae39e9de","role":"authenticated"}', true);

DO $$
DECLARE
  v_empresa uuid := 'dff833d4-5dd8-499b-b4ad-bc04d85d8172';
  v_user uuid := 'cdd5a5ad-cb74-45fa-9deb-79faae39e9de';
...
BEGIN
  ...
  INSERT INTO riscos_matrizes (nome, descricao, empresa_id) VALUES ('Matriz Corporativa CyberMe 2026','Matriz principal',v_empresa) RETURNING id INTO v_matriz;
  ...
  INSERT INTO sistemas_privilegiados (nome_sistema, tipo_sistema, criticidade, empresa_id, responsavel_sistema) VALUES ('Active Directory CyberMe','Autenticação','critico',v_empresa,'TI') RETURNING id INTO v_sis;
  ...
  INSERT INTO riscos (empresa_id, nome, descricao, categoria_id, matriz_id, probabilidade_inicial, impacto_inicial, nivel_risco_inicial, status, responsavel, data_identificacao, created_by) VALUES
    (v_empresa,'Vazamento de Dados de Clientes','Exposição de dados pessoais',v_cat_risco,v_matriz,'provavel','catastrofico','critico','tratado','Halan Borges - CISO',CURRENT_DATE-45,v_user),
    (v_empresa,'Ataque Ransomware','Sequestro de dados produção',v_cat_risco,v_matriz,'possivel','catastrofico','critico','monitorado','Halan Borges - CISO',CURRENT_DATE-90,v_user),
    ...
    (v_empresa,'Acesso Não Autorizado','Tentativas acesso indevido',v_cat_risco,v_matriz,'provavel','maior','alto','tratado','Halan Borges - CISO',CURRENT_DATE-30,v_user),
    ...
    (v_empresa,'Shadow IT','Software não autorizado',v_cat_risco,v_matriz,'provavel','moderado','medio','em_tratamento','Halan Borges - CISO',CURRENT_DATE-35,v_user);
  ...
  INSERT INTO denuncias (empresa_id, token_publico, protocolo, categoria_id, titulo, descricao, status, gravidade, anonima) VALUES
    (v_empresa, replace(gen_random_uuid()::text,'-',''), 'DEN'||to_char(NOW(),'YYYYMMDD')||'0001', v_cat_den,'Conduta inadequada gestor','Relato em reunião','em_analise','alta',true),
    (v_empresa, replace(gen_random_uuid()::text,'-',''), 'DEN'||to_char(NOW(),'YYYYMMDD')||'0002', v_cat_den,'Discriminação','Processo seletivo','em_investigacao','alta',true),
    (v_empresa, replace(gen_random_uuid()::text,'-',''), 'DEN'||to_char(NOW(),'YYYYMMDD')||'0003', v_cat_den,'Uso indevido recursos','Recursos para fins pessoais','resolvida','media',false);
  ...
  INSERT INTO ativos (empresa_id, nome, tipo, descricao, proprietario, localizacao, valor_negocio, criticidade, status, data_aquisicao, fornecedor, versao) VALUES
    ...
    (v_empresa,'Notebook Halan','tecnologia','Notebook executivo','Halan Borges','Escritório','8500','alto','ativo',CURRENT_DATE-200,'Dell','Latitude 7430'),
  ...
  INSERT INTO contas_privilegiadas (sistema_id, empresa_id, usuario_beneficiario, email_beneficiario, tipo_acesso, nivel_privilegio, data_concessao, status, justificativa_negocio) VALUES
    (v_sis,v_empresa,'admin-ad','admin-ad@cyberme.com.br','permanente','administrador',CURRENT_DATE-365,'ativo','Administração do domínio Active Directory'),
    (v_sis,v_empresa,'svc-backup','svc-backup@cyberme.com.br','permanente','servico',CURRENT_DATE-200,'ativo','Service account para rotinas de backup automatizado'),
    (v_sis,v_empresa,'root-prod','root@cyberme.com.br','emergencial','administrador',CURRENT_DATE-100,'ativo','Acesso root para administração de servidores produção'),
    (v_sis,v_empresa,'dba-postgres','dba@cyberme.com.br','permanente','administrador',CURRENT_DATE-300,'ativo','DBA para administração do cluster PostgreSQL');
END $$;
