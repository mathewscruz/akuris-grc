    ...
    case "critico":
      return (
        <StatusBadge tone="destructive" intensity="high">
          {t("dadosDashboard.descobertaDadosTab.sensitivityCritico")}
        </StatusBadge>
      );
    case "sensivel":
      return (
        <StatusBadge tone="warning">
          {t("dadosDashboard.descobertaDadosTab.sensitivitySensivel")}
        </StatusBadge>
      );
    default:
      return (
        <StatusBadge tone="neutral">
          {t("dadosDashboard.descobertaDadosTab.sensitivityComum")}
        </StatusBadge>
      );
  }
};
...
const getCategoryLabel = (category: string, t: (key: string) => string) => {
  const labels: Record<string, string> = {
    identificacao: t(
      "dadosDashboard.descobertaDadosTab.categoriaIdentificacao",
    ),
...
export function DescoberDadosTab({
  onRefresh,
  canCreate = false,
  canDelete = false,
}: DescoberDadosTabProps) {
  const { t } = useLanguage();
  ...
  const [selectedDescoberta, setSelectedDescoberta] =
    useState<Descoberta | null>(null);
  ...
  const confirmDelete = async () => {
    try {
      ...
      toast({
        ...
        description: t(
          "dadosDashboard.descobertaDadosTab.toastDeleteSuccessDescription",
        ),
      });
      ...
      toast({
        ...
        description:
          error.message ||
          t("dadosDashboard.descobertaDadosTab.toastDeleteErrorDefault"),
        variant: "destructive",
      });
    }
  };
  ...
  const handleScanComplete = async (
    ...
    try {
      ...
      await exigirEscrita(
        supabase.from("dados_descobertas").insert({
      ...
      render: (value: string, row: Descoberta) => (
        <div className="max-w-[200px]">
          <p className="font-medium truncate">
            {value || t("dadosDashboard.descobertaDadosTab.semTitulo")}
          </p>
          <a
            href={row.url}
            target="_blank"
          ...
          <div className="flex items-center gap-1">
            {canDelete && (
              <Tooltip>
                ...
                <TooltipContent>
                  {t("dadosDashboard.descobertaDadosTab.tooltipVerDetalhes")}
                </TooltipContent>
              </Tooltip>
            )}
            <Tooltip>
              ...
              <TooltipContent>
                {t("dadosDashboard.descobertaDadosTab.tooltipExcluir")}
              </TooltipContent>
            </Tooltip>
          </div>
        </TooltipProvider>
    ...
    <Card className="rounded-lg border">
      <CardContent className="p-0">
        <div className="p-6 pb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h3 className="text-lg font-semibold">
              {t("dadosDashboard.descobertaDadosTab.headerTitle")}
            </h3>
        ...
        <DataTable
          ...
          columns={columns}
          searchPlaceholder={t(
            "dadosDashboard.descobertaDadosTab.searchPlaceholder",
          )}
          sortField={sortField}
          ...
          emptyState={{
            ...
            description: t(
              "dadosDashboard.descobertaDadosTab.emptyDescription",
            ),
            action: canCreate
              ? {
                  label: t(
                    "dadosDashboard.descobertaDadosTab.buttonNovaDescoberta",
                  ),
      ...
      {canCreate && (
        <UrlScannerDialog
          ...
          onImport={async (fields, scanResult) => {
            if (!empresaId)
              throw new Error(
                t("dadosDashboard.common.errorEmpresaNaoEncontrada"),
              );
            ...
            const normalizar = (valor: string) =>
              valor.trim().toLocaleLowerCase();
            const nomes = new Set(
              (existentes || []).map((d: any) => normalizar(d.nome || "")),
            );
            const novos: any[] = [];
            for (const field of fields) {
              const nome =
                ...
                field.name ||
                field.id ||
                t("dadosDashboard.descobertaDadosTab.importCampoDefault", {
                  dataType: field.dataType,
                });
              const chave = normalizar(nome);
              if (!chave || nomes.has(chave)) continue;
              nomes.add(chave);
              novos.push({
                empresa_id: empresaId,
                ...
                nome: nome,
                descricao: t(
                  "dadosDashboard.descobertaDadosTab.importDescricao",
                  {
                    field: field.name || field.id,
                    placeholder: field.placeholder
                      ? t(
                          "dadosDashboard.descobertaDadosTab.importPlaceholderSuffix",
                          { placeholder: field.placeholder },
                        )
                      : "",
                  },
                ...
                tipo_dados:
                  field.sensitivity === "critico" ? "sensivel" : "comum",
                sensibilidade:
                  field.sensitivity === "critico"
                    ? "muito_sensivel"
                    : field.sensitivity === "sensivel"
                      ? "sensivel"
                      : "comum",
                ...
                base_legal: null,
                nivel_catalogo: "campo",
                origem_validada: false,
              });
            }
            if (novos.length > 0) {
              await exigirEscrita(
                supabase.from("dados_pessoais").insert(novos as any),
              );
            }
            const created = novos.length;
            ...
            toast({
              title: t(
                "dadosDashboard.descobertaDadosTab.toastImportSuccessTitle",
              ),
              description: t(
                "dadosDashboard.descobertaDadosTab.toastImportSuccessDescription",
                { count: created },
              ),
            });
            onRefresh();
      ...
      >
        {selectedDescoberta && (
          <div className="space-y-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-2xl font-bold">
                        {selectedDescoberta.total_formularios}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t("dadosDashboard.descobertaDadosTab.cardFormularios")}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-2xl font-bold">
                        {selectedDescoberta.total_campos}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t("dadosDashboard.descobertaDadosTab.cardCampos")}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-2xl font-bold">
                        {selectedDescoberta.campos_sensiveis}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t("dadosDashboard.descobertaDadosTab.cardSensiveis")}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <Card>
                <CardContent className="pt-4">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <p className="text-2xl font-bold">
                        {selectedDescoberta.campos_criticos}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {t("dadosDashboard.descobertaDadosTab.cardCriticos")}
                      </p>
                    </div>
                  </div>
                </CardContent>
              ...
              <CardContent className="py-3">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <span className="font-medium">
                    {selectedDescoberta.titulo_pagina ||
                      t("dadosDashboard.descobertaDadosTab.semTitulo")}
                  </span>
                </div>
                <a
                ...
                <div className="mt-2 text-xs text-muted-foreground">
                  {t("dadosDashboard.descobertaDadosTab.scannedAtPrefix")}:{" "}
                  {formatDateOnly(selectedDescoberta.created_at)}
                  {selectedDescoberta.campos_importados > 0 && (
                    <>
                      {" "}
                      • {selectedDescoberta.campos_importados}{" "}
                      {t(
                        "dadosDashboard.descobertaDadosTab.importedFieldsSuffix",
                      )}
                    </>
                  )}
                </div>
            ...
            {/* Forms Accordion */}
            {selectedDescoberta.resultado_scan &&
              selectedDescoberta.resultado_scan.length > 0 && (
                    ...
                    <AccordionItem key={formIndex} value={`form-${formIndex}`}>
                      <AccordionTrigger className="hover:no-underline">
                        <div className="flex items-center gap-3">
                          ...
                          <StatusBadge tone="neutral">
                            {t(
                              "dadosDashboard.descobertaDadosTab.camposCount",
                        ...
                        <Table>
                          <TableHeader>
                            <TableRow>
                              <TableHead>
                                {t(
                                  "dadosDashboard.descobertaDadosTab.campoLabel",
                                )}
                              </TableHead>
                              <TableHead>
                                {t(
                                  "dadosDashboard.descobertaDadosTab.tipoHtmlLabel",
                                )}
                              </TableHead>
                              <TableHead>
                                {t(
                                  "dadosDashboard.descobertaDadosTab.tipoDadoLabel",
                                )}
                              </TableHead>
                              <TableHead>
                                {t(
                                  "dadosDashboard.descobertaDadosTab.categoriaLgpdLabel",
                                )}
                              </TableHead>
                              <TableHead>
                                {t(
                                  "dadosDashboard.descobertaDadosTab.sensibilidadeLabel",
                                )}
                              </TableHead>
                            </TableRow>
                          </TableHeader>
                          <TableBody>
                            {form.fields.map((field, fieldIndex) => (
                              <TableRow key={fieldIndex}>
                                  ...
                                  <div>
                                    <p className="font-medium">
                                      {field.label ||
                                        ...
                                        field.id ||
                                        t(
                                          "dadosDashboard.descobertaDadosTab.semNome",
                                        )}
                                    </p>
                                    {field.placeholder && (
                                      <p className="text-xs text-muted-foreground">
                                ...
                                <TableCell>
                                  {getDataTypeLabel(field.dataType, t)}
                                </TableCell>
                                <TableCell>
                                  {getCategoryLabel(field.lgpdCategory, t)}
                                </TableCell>
                                <TableCell>
                                  {getSensitivityBadge(field.sensitivity, t)}
                                </TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
      ...
      <ConfirmDialog
        ...
        description={t(
          "dadosDashboard.descobertaDadosTab.deleteDialogDescription",
        )}
