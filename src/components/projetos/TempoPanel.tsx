...
import { IconAdd, IconDelete, IconPlay, IconPause, IconTimer } from '@/components/icons';
  ...
  return (
    <div className="space-y-4">
      <div className="rounded-lg border border-border bg-card p-4">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <IconTimer className="h-4 w-4 text-primary" strokeWidth={1.5} />
            <span className="text-sm font-semibold">{t('projetos.tempo.timer')}</span>
          </div>
          {startedAt && <StatusBadge tone="info">{fmtTimer(elapsed)}</StatusBadge>}
        </div>
        <div className="flex gap-2 items-start">
          ...
          {startedAt ? (
            <Button size="sm" variant="destructive" onClick={stopAndSave} disabled={add.isPending}>
              <IconPause className="h-4 w-4" /> {t('projetos.tempo.stopAndSave')}
            </Button>
          ) : (
            <Button size="sm" onClick={start}>
              <IconPlay className="h-4 w-4" /> {t('projetos.tempo.start')}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="space-y-1.5">
        <div className="text-xs font-semibold text-muted-foreground">{t('projetos.tempo.history')}</div>
        {entradas.length === 0 ? (
            ...
            {entradas.map((e) => (
              <li key={e.id} className="flex items-center justify-between gap-2 px-3 py-2 text-sm group">
                <div className="flex-1 min-w-0">
                  <div className="font-medium">{Number(e.horas).toFixed(2)}h <span className="text-muted-foreground font-normal">· {new Date(e.data).toLocaleDateString(intlLocale())}</span></div>
