export const pt = {
  language: {
    selector: 'Idioma',
  },
  common: {
    selectDate: 'Selecionar data',
    save: 'Salvar',
    saving: 'Salvando...',
    cancel: 'Cancelar',
    delete: 'Excluir',
    deleting: 'Excluindo...',
    edit: 'Editar',
    new: 'Novo',
    create: 'Criar',
    creating: 'Criando...',
    view: 'Visualizar',
    download: 'Baixar',
    upload: 'Enviar',
    import: 'Importar',
    submit: 'Enviar',
    export: 'Exportar',
    search: 'Buscar',
    searchPlaceholder: 'Buscar...',
    filter: 'Filtrar',
    filters: 'Filtros',
    clearFilters: 'Limpar filtros',
    apply: 'Aplicar',
    reset: 'Redefinir',
    add: 'Adicionar',
    remove: 'Remover',
    loading: 'Carregando...',
    retry: 'Tentar novamente',
    /* Um período não acaba antes de começar. Havia duas versões desta
       frase escritas à mão, em dois módulos; os outros quatro nem
       validavam. */
    fimAntesDoInicio: 'A data de fim tem de ser igual ou posterior à de início.',
    noResults: 'Nenhum resultado encontrado',
    noResultsHint: 'Ajuste a busca ou os filtros para ver mais registros.',
    noData: 'Sem dados',
    confirm: 'Confirmar',
    close: 'Fechar',
    back: 'Voltar',
    next: 'Próximo',
    previous: 'Anterior',
    finish: 'Finalizar',
    yes: 'Sim',
    no: 'Não',
    actions: 'Ações',
    status: 'Status',
    date: 'Data',
    name: 'Nome',
    description: 'Descrição',
    type: 'Tipo',
    priority: 'Prioridade',
    responsible: 'Responsável',
    observations: 'Observações',
    attention: 'Atenção',
    ok: 'OK',
    update: 'Atualizar',
    generate: 'Gerar',
    pdf: 'PDF',
    required: 'Obrigatório',
    optional: 'Opcional',
    select: 'Selecionar',
    selectPlaceholder: 'Selecione...',
    clear: 'Limpar',
    details: 'Detalhes',
    summary: 'Resumo',
    total: 'Total',
    today: 'Hoje',
    yesterday: 'Ontem',
    tomorrow: 'Amanhã',
    unknown: 'Desconhecido',
    notDefined: 'Não definido',
    error: 'Erro',
    success: 'Sucesso',
    warning: 'Aviso',
    info: 'Informação',
    tryAgain: 'Tentar novamente',
    copy: 'Copiar',
    copied: 'Copiado!',
    sending: 'Enviando...',
    saved: 'Salvo',
    savedAt: 'Salvo às {time}',
    of: 'de',
    page: 'Página',
    rowsPerPage: 'Linhas por página',
    selectAll: 'Selecionar todos',
    all: 'Todos',
    none: 'Nenhum',
    open: 'Abrir',
    refresh: 'Atualizar',
    settings: 'Configurações',
    help: 'Ajuda',
    learnMore: 'Saiba mais',
    showDetails: 'Ver detalhes',
    hideDetails: 'Ocultar detalhes',
    undo: 'Desfazer',
  },
  validation: {
    required: 'Campo obrigatório',
    invalidEmail: 'E-mail inválido',
    invalidUrl: 'URL inválida',
    invalidDate: 'Data inválida',
    minLength: 'Mínimo de {min} caracteres',
    maxLength: 'Máximo de {max} caracteres',
    minValue: 'Valor mínimo: {min}',
    maxValue: 'Valor máximo: {max}',
    passwordsMustMatch: 'As senhas não coincidem',
    invalidNumber: 'Número inválido',
    invalidFormat: 'Formato inválido',
  },
  toasts: {
    saveSuccess: 'Salvo com sucesso',
    saveError: 'Erro ao salvar',
    deleteSuccess: 'Excluído com sucesso',
    deleteError: 'Erro ao excluir',
    updateSuccess: 'Atualizado com sucesso',
    updateError: 'Erro ao atualizar',
    createSuccess: 'Criado com sucesso',
    createError: 'Erro ao criar',
    loadError: 'Erro ao carregar dados',
    unauthorized: 'Não autorizado',
    networkError: 'Erro de conexão',
    genericError: 'Ocorreu um erro. Tente novamente.',
    copySuccess: 'Copiado para a área de transferência',
    sessionExpired: 'Sessão expirada. Faça login novamente.',
  },
  dialogs: {
    unsavedChanges: 'Alterações não salvas',
    unsavedChangesDesc: 'Você tem alterações não salvas. Deseja sair mesmo assim?',
    confirmDelete: 'Confirmar exclusão',
    confirmDeleteDesc: 'Tem certeza que deseja excluir este item? Esta ação não pode ser desfeita.',
    confirmAction: 'Confirmar ação',
    cannotUndo: 'Esta ação não pode ser desfeita.',
    discard: 'Descartar',
    keepEditing: 'Continuar editando',
  },
  wizard: {
    step: 'Etapa',
    stepOf: 'Etapa {current} de {total}',
    previous: 'Anterior',
    next: 'Próximo',
    finish: 'Finalizar',
    incomplete: 'Incompleto',
    partial: 'Parcial',
    complete: 'Completo',
    pending: 'Pendente',
    error: 'Erro',
    saveDraft: 'Salvar rascunho',
    draftSaved: 'Rascunho salvo',
  },
  tables: {
    noRows: 'Nenhum registro encontrado',
    noFilterMatch: 'Nenhum resultado para os filtros aplicados',
    selectAll: 'Selecionar todos',
    selected: '{count} selecionado(s)',
    rowsPerPage: 'Linhas por página',
    pageOf: 'Página {current} de {total}',
    loading: 'Carregando registros...',
    showing: 'Mostrando {from}-{to} de {total}',
  },
  emptyStates: {
    noItems: 'Nenhum item ainda',
    getStarted: 'Comece criando seu primeiro item',
    createFirst: 'Criar primeiro item',
    noFilterResults: 'Nenhum resultado para os filtros',
    tryDifferentFilter: 'Tente ajustar os filtros',
  },
  sidebar: {
    dashboard: 'Dashboard',
    actionPlans: 'Planos de Ação',
    projects: 'Projetos',
    assetManagement: 'Gestão de Ativos',
    assets: 'Ativos',
    licenses: 'Licenças',
    keys: 'Chaves',
    riskManagement: 'Gestão de Riscos',
    risks: 'Riscos',
    riskAcceptance: 'Aceite de Risco',
    gapAnalysis: 'Gap Analysis',
    frameworks: 'Frameworks',
    adherenceAssessment: 'Avaliação de Aderência',
    governance: 'Governança',
    internalControls: 'Controles Internos',
    audits: 'Auditorias',
    systems: 'Sistemas',
    accessManagement: 'Gestão de Acessos',
    contracts: 'Contratos',
    documents: 'Documentos',
    security: 'Segurança',
    privilegedAccounts: 'Contas Privilegiadas',
    accessReview: 'Revisão de Acessos',
    incidents: 'Incidentes',
    privacy: 'Privacidade',
    compliance: 'Compliance',
    dueDiligence: 'Due Diligence',
    whistleblowing: 'Canal de Denúncia',
    
    reports: 'Relatórios',
    businessContinuity: 'Continuidade',
    settings: 'Configurações',
    logout: 'Sair',
    confirmLogout: 'Confirmar saída',
    confirmLogoutDesc: 'Tem certeza que deseja sair do sistema?',
    signOutFailed: 'Não foi possível encerrar a sessão. Tente novamente.',
    sectionOperation: 'Operação',
    sectionGrcCore: 'Governança e risco',
    sectionCompliance: 'Proteção e continuidade',
    sectionInsights: 'Insights',
  },
  theme: {
    appearance: 'Aparência',
    toggleLight: 'Modo claro',
    toggleDark: 'Modo escuro',
  },
  dashboard_v3: {
    greetingMorning: 'Bom dia',
    greetingAfternoon: 'Boa tarde',
    greetingEvening: 'Boa noite',
    criticalSummary: '{{count}} item crítico exige sua atenção',
    criticalSummaryPlural: '{{count}} itens críticos exigem sua atenção',
    allClear: 'Nada crítico no momento',
    updatedAt: 'Atualizado às {{time}}',
    refresh: 'Atualizar indicadores',
    refreshing: 'Atualizando…',
    refreshed: 'Atualizado',
    /* A acção primária da página: o painel não tinha nenhuma. */
    executiveReport: 'Relatório executivo',
    hello: 'Olá, {name}',
    /** O único agregado que atravessa todos os módulos, e a porta do detalhe. */
    criticalAction: {
      one: '1 alerta crítico por decidir',
      other: '{count} alertas críticos por decidir',
    },
    noCritical: 'Nenhum alerta crítico',
  },
  userProfile: {
    myProfile: 'Meu perfil',
    editProfile: 'Editar perfil',
    changePassword: 'Alterar senha',
    signOut: 'Sair',
    notifications: 'Notificações',
    preferences: 'Preferências',
    language: 'Idioma',
    portuguese: 'Português',
    english: 'Inglês',
  },
  mfa: {
    title: 'Verificação em duas etapas',
    description: 'Digite o código enviado para seu e-mail',
    code: 'Código',
    codePlaceholder: 'Digite o código de 6 dígitos',
    verify: 'Verificar',
    verifying: 'Verificando...',
    resend: 'Reenviar código',
    resendIn: 'Reenviar em {seconds}s',
    invalidCode: 'Código inválido',
    expired: 'Código expirado',
    cancel: 'Cancelar',
  },
  trial: {
    daysLeft: '{days} dias restantes',
    expired: 'Período de teste expirado',
    expiringToday: 'Seu teste expira hoje',
    upgradeNow: 'Fazer upgrade',
    contactSales: 'Falar com vendas',
  },
  plan: {
    free: 'Gratuito',
    pro: 'Pro',
    enterprise: 'Enterprise',
    currentPlan: 'Plano atual',
    upgrade: 'Fazer upgrade',
    manageSubscription: 'Gerenciar assinatura',
  },
  passwordChange: {
    title: 'Alteração de Senha Obrigatória',
    description: 'Por segurança, você deve alterar sua senha temporária antes de continuar.',
    alertMessage: 'Esta é a primeira vez que você acessa o sistema. Por favor, defina uma nova senha segura.',
    currentPassword: 'Senha Atual (Temporária)',
    currentPasswordPlaceholder: 'Digite sua senha temporária',
    newPassword: 'Nova Senha',
    newPasswordPlaceholder: 'Digite sua nova senha',
    confirmPassword: 'Confirmar Nova Senha',
    confirmPasswordPlaceholder: 'Confirme sua nova senha',
    save: 'Alterar Senha',
    saving: 'Alterando senha...',
    success: 'Senha alterada com sucesso!',
    error: 'Erro ao alterar senha',
    fillAllFields: 'Por favor, preencha todos os campos',
    passwordsDontMatch: 'As senhas não coincidem',
    minLengthError: 'A nova senha deve ter pelo menos 8 caracteres',
    mustBeDifferent: 'A nova senha deve ser diferente da senha atual',
    incorrectCurrent: 'Senha atual incorreta. Verifique e tente novamente.',
    requirements: 'Requisitos da senha:',
    reqMinLength: 'Mínimo 8 caracteres',
    reqDifferent: 'Diferente da senha atual',
    reqMatch: 'Senhas coincidem',
    strengthVeryWeak: 'Muito fraca',
    strengthWeak: 'Fraca',
    strengthFair: 'Regular',
    strengthGood: 'Boa',
    strengthStrong: 'Forte',
    successRelogin: 'Senha alterada. Entre novamente para iniciar uma sessão protegida.',
  },
  userProfilePopover: {
    tabProfile: 'Perfil',
    tabSecurity: 'Segurança',
    tabNotifications: 'Notificações',
    eyebrow: 'Conta',
    emailLabel: 'E-mail',
    emailHint: 'O e-mail de acesso não pode ser alterado aqui.',
    passwordHint: 'Deixe em branco para manter a senha atual.',
    changePhoto: 'Alterar Foto',
    photoFormats: 'JPG, PNG, GIF, SVG, WebP (máx. 5MB)',
    name: 'Nome',
    namePlaceholder: 'Seu nome completo',
    nameRequired: 'Nome é obrigatório',
    changePassword: 'Alterar Senha',
    currentPassword: 'Senha Atual',
    currentPasswordPlaceholder: 'Digite sua senha atual',
    newPassword: 'Nova Senha',
    newPasswordPlaceholder: 'Digite a nova senha',
    confirmNewPassword: 'Confirmar Nova Senha',
    confirmNewPasswordPlaceholder: 'Confirme a nova senha',
    saveChanges: 'Salvar Alterações',
    saving: 'Salvando...',
    photoUpdated: 'Foto atualizada com sucesso',
    photoError: 'Erro ao fazer upload da foto',
    profileUpdated: 'Perfil atualizado com sucesso',
    profileError: 'Erro ao atualizar perfil',
    incorrectCurrentPassword: 'Senha atual incorreta. Verifique e tente novamente.',
    invalidImageFormat: 'Formato de arquivo não suportado. Use: JPG, PNG, GIF, SVG ou WebP',
    fileTooLarge: 'Arquivo muito grande. Tamanho máximo: 5MB',
    passwordsAndCurrent: 'As senhas não coincidem ou senha atual não foi informada',
    newPasswordMin: 'A nova senha deve ter pelo menos 8 caracteres',
    notifications: 'Notificações',
    prefsErro: 'Não foi possível gravar a preferência.',
    emailNotif: 'Email',
    emailNotifDesc: 'Alertas por email',
    marketingEmails: 'Comunicados da Akuris',
    marketingEmailsDesc: 'Novidades de produto e conteúdo editorial. Alertas da conta continuam ativos.',
    inAppNotif: 'In-App',
    inAppNotifDesc: 'Notificações no sino',
    frequency: 'Frequência',
    realtime: 'Tempo Real',
    daily: 'Diário',
    weekly: 'Semanal',
    activeSessions: 'Sessões ativas',
    activeSessionsDescription: 'Desconecte outros navegadores e dispositivos, preservando este acesso.',
    endOtherSessions: 'Encerrar outras sessões',
    endingOtherSessions: 'Encerrando...',
    otherSessionsEnded: 'As outras sessões foram encerradas.',
    otherSessionsError: 'Não foi possível encerrar todas as outras sessões.',
  },
  mfaScreen: {
    title: 'Verificação de Segurança',
    description: 'Enviamos um código de 6 dígitos para:',
    verify: 'Verificar Código',
    verifying: 'Verificando...',
    resending: 'Reenviando...',
    resendCode: 'Reenviar código',
    resendIn: 'Reenviar em {seconds}s',
    cancelBack: 'Cancelar e voltar ao login',
    enterFullCode: 'Digite o código completo de 6 dígitos',
    invalidCode: 'Código inválido',
    verifyError: 'Erro ao verificar código',
    newCodeSent: 'Novo código enviado!',
    resendError: 'Erro ao reenviar código',
    /* O que se diz quando o envio NÃO aconteceu. A frase de cima —
       «Enviamos um código» — era mostrada mesmo assim. */
    envioFalhouTitulo: 'Não conseguimos enviar o código',
    envioFalhouDescricao: 'O envio para {email} não foi concluído. Nenhum código foi gerado — use o botão abaixo para tentar de novo.',
    allRightsReserved: 'Todos os direitos reservados',
    eyebrow: 'Verificação em duas etapas',
    heading: 'Confirme que é você',
    securityFootnote: 'Código válido por 5 minutos · Nunca compartilhe',
    backToLogin: 'Voltar ao login',
    step: 'Etapa 2 de 2',
    descriptionWithEmail: 'Digite o código de 6 dígitos enviado para {email}.',
    codeValidity: 'Validade do código',
    codeInputLabel: 'Código de verificação de 6 dígitos',
    invalidWithAttempts: 'Código incorreto. Restam {attempts} tentativas.',
    expiredCode: 'Este código expirou. Solicite um novo código para continuar.',
    tooManyAttempts: 'O código foi bloqueado após muitas tentativas. Solicite outro.',
    sessionExpired: 'Sua sessão de login expirou. Volte e entre novamente.',
    waitBeforeResend: 'Aguarde o tempo indicado antes de solicitar outro código.',
    rateLimited: 'Muitos códigos foram solicitados. Aguarde antes de tentar novamente.',
  },
  changelog: {
    title: 'Novidades',
    subtitle: 'Últimas atualizações da plataforma',
    whatsNew: 'O que há de novo',
    viewAll: 'Ver todas',
    new: 'Novo',
    feature: 'Novo',
    improved: 'Melhoria',
    improvement: 'Melhoria',
    fixed: 'Correção',
    fix: 'Correção',
    noUpdates: 'Nenhuma atualização disponível',
    close: 'Fechar',
    tabAlerts: 'Alertas',
    tabNews: 'Novidades',
  },
  trialBanner: {
    daysLeftSingular: 'Licença Trial - Resta 1 dia de teste.',
    daysLeftPlural: 'Licença Trial - Restam {days} dias de teste.',
    viewPlans: 'Ver planos',
  },
  creditsExhausted: {
    title: 'Créditos de IA Esgotados',
    description: 'Sua empresa atingiu o limite de {limit} créditos de IA do plano {plan}.',
    currentPlan: 'seu plano atual',
    toContinue: 'Para continuar utilizando funcionalidades de IA:',
    contactItem: 'Entre em contato com a Akuris',
    upgradeItem: 'Solicite upgrade de plano ou créditos extras',
    understood: 'Entendi',
  },
  commandPalette: {
    placeholder: 'Buscar comandos, módulos, ações...',
    noResults: 'Nenhum resultado encontrado',
    noResultsFound: 'Nenhum resultado encontrado.',
    navigation: 'Navegação',
    actions: 'Ações',
    recent: 'Recentes',
    modules: 'Módulos',
    keyboardShortcuts: 'Atalhos de Teclado',
    quickSearch: 'Busca Rápida',
    toggleMenu: 'Abrir/Fechar Menu',
    searchButton: 'Buscar...',
    moduleDashboard: 'Dashboard',
    moduleRisks: 'Gestão de Riscos',
    moduleGovernance: 'Governança',
    moduleGapAnalysis: 'Gap Analysis',
    moduleFrameworks: 'Frameworks',
    moduleAssets: 'Ativos',
    moduleLicenses: 'Licenças',
    moduleCryptoKeys: 'Chaves Criptográficas',
    moduleDocuments: 'Documentos',
    moduleContracts: 'Contratos',
    moduleIncidents: 'Incidentes',
    modulePrivacy: 'Privacidade (LGPD)',
    modulePrivilegedAccounts: 'Contas Privilegiadas',
    moduleAccessReviews: 'Revisão de Acessos',
    moduleDueDiligence: 'Due Diligence',
    moduleWhistleblowing: 'Canal de Denúncias',
    moduleActionPlans: 'Planos de Ação',
    moduleReports: 'Relatórios',
    moduleSettings: 'Configurações',
  },
  defineSenhaPage: {
    title: 'Defina sua Senha',
    subtitle: 'Crie uma senha segura para sua conta',
    verifying: 'Verificando link...',
    invalidLinkTitle: 'Link Inválido ou Expirado',
    invalidLinkDesc: 'Este link expirou ou já foi utilizado. Solicite um novo para continuar; cada link é válido por 1 hora.',
    requestNewLink: 'Pedir novo link',
    goToLogin: 'Ir para Login',
    newPassword: 'Nova Senha',
    newPasswordPlaceholder: 'Mínimo 8 caracteres',
    confirmPassword: 'Confirmar Senha',
    confirmPasswordPlaceholder: 'Repita a senha',
    requirements: 'Requisitos da senha:',
    reqMinChars: 'Mínimo 8 caracteres',
    reqUppercase: 'Uma letra maiúscula',
    reqLowercase: 'Uma letra minúscula',
    reqNumber: 'Um número',
    submit: 'Definir Senha',
    saving: 'Salvando...',
    successTitle: 'Senha Definida!',
    successDesc: 'Todas as sessões anteriores foram encerradas. Você voltará ao login em {seconds}s.',
    success: 'Senha definida com sucesso!',
    error: 'Erro ao definir senha',
    grcSubtitle: 'Governança, Risco e Compliance',
    allRightsReserved: 'Todos os direitos reservados',
    validationMin: 'Senha deve ter no mínimo 8 caracteres',
    validationUpper: 'Deve conter pelo menos uma letra maiúscula',
    validationLower: 'Deve conter pelo menos uma letra minúscula',
    validationNumber: 'Deve conter pelo menos um número',
    validationMatch: 'As senhas não coincidem',
    linkStatus: 'Segurança da conta',
    completed: 'Alteração concluída',
    signInNow: 'Entrar com a nova senha',
    eyebrow: 'Nova credencial',
    secureSession: 'Link verificado',
  },
  checkoutSuccessPage: {
    title: 'Assinatura confirmada!',
    redirectMessage: 'Sua conta está pronta. Você será redirecionado para o dashboard em {seconds} segundos...',
    notLoggedMessage: 'Sua assinatura foi confirmada com sucesso! Faça login para acessar o sistema.',
    loginButton: 'Fazer login',
  },
  notFoundPage: {
    title: '404',
    subtitle: 'Ops! Página não encontrada',
    returnHome: 'Voltar ao início',
  },
  filterBar: {
    filters: 'Filtros',
    clear: 'Limpar',
    all: 'Todos',
    activeFilters: 'Filtros ativos:',
    search: 'Busca',
  },
  empty: {
    noUpdatesAvailable: 'Nenhuma atualização disponível',
  },
  layout: {
    blockedTitle: 'Acesso Bloqueado',
    blockedDesc: 'Sua empresa está temporariamente desativada. Entre em contato com o suporte para mais informações.',
    contactSupport: 'Entrar em Contato',
    signOut: 'Sair',
    trialExpiredTitle: 'Período de Teste Expirado',
    trialExpiredDesc: 'Seu período de teste de 14 dias chegou ao fim. Entre em contato para ativar sua licença e continuar usando o Akuris.',
    activateLicense: 'Ativar Licença',
    backTooltip: 'Voltar',
    moreOptions: 'Mais opções',
  },
  errorBoundary: {
    title: 'Algo deu errado',
    description: 'Ocorreu um erro inesperado. Tente recarregar a página.',
    reload: 'Recarregar',
    backToHome: 'Voltar ao início',
  },
  notFound: {
    title: 'Página não encontrada',
    description: 'A página que você procura não existe ou foi movida.',
    backToHome: 'Voltar ao início',
  },
  checkoutSuccess: {
    title: 'Pagamento confirmado!',
    description: 'Sua assinatura foi ativada com sucesso.',
    goToDashboard: 'Ir para o Dashboard',
  },
  defineSenha: {
    title: 'Definir senha',
    description: 'Crie uma senha segura para acessar sua conta.',
    newPassword: 'Nova senha',
    confirmPassword: 'Confirmar senha',
    save: 'Definir senha',
    saving: 'Salvando...',
    success: 'Senha definida com sucesso',
    error: 'Erro ao definir senha',
    invalidLink: 'Link inválido ou expirado',
  },
  auth: {
    systemAccess: 'Acesso ao Sistema',
    enterCredentials: 'Entre com suas credenciais',
    email: 'E-mail',
    emailPlaceholder: 'email@empresa.com.br',
    password: 'Senha',
    rememberMe: 'Lembrar-me',
    forgotPassword: 'Esqueci minha senha',
    signIn: 'Entrar',
    signingIn: 'Entrando...',
    success: 'Sucesso!',
    loginSuccess: 'Login realizado com sucesso!',
    loginCancelled: 'Login cancelado',
    noAccount: 'Não tem uma conta?',
    createFreeAccount: 'Criar conta grátis',
    privacyPolicy: 'Política de Privacidade',
    allRightsReserved: 'Todos os direitos reservados',
    platformTitle: 'Sempre pronto',
    platformHighlight: 'para a auditoria.',
    featureGovernance: 'Governança',
    featureGovernanceDesc: 'Políticas, documentos e conformidade centralizada',
    featureRisks: 'Riscos',
    featureRisksDesc: 'Matriz de riscos e tratamentos integrados',
    featureCompliance: 'Compliance',
    featureComplianceDesc: 'Gap analysis, auditorias e controles',
    mobileSubtitle: 'Governança, Risco e Compliance',
    errorInvalidCredentials: 'Email ou senha incorretos',
    errorEmailNotConfirmed: 'Email não confirmado. Verifique sua caixa de entrada.',
    errorUserNotFound: 'Usuário não encontrado',
    errorTooManyRequests: 'Muitas tentativas. Aguarde alguns minutos.',
    errorNetwork: 'Erro de conexão. Verifique sua internet.',
    errorGeneric: 'Erro ao fazer login. Tente novamente.',
    errorAuthAfterMFA: 'Erro ao autenticar após verificação. Tente novamente.',
    errorAuth: 'Erro ao autenticar. Tente novamente.',
    validationEmailRequired: 'Email é obrigatório',
    validationEmailInvalid: 'Email inválido',
    validationPasswordMin: 'Senha deve ter no mínimo 8 caracteres',
    validationPasswordRequired: 'Senha é obrigatória',
    eyebrowPlatform: 'PLATAFORMA GRC',
    eyebrowWelcome: 'BEM-VINDO DE VOLTA',
    heading: 'Entre na sua conta',
    headingSub: 'Acesse o Command Center da sua operação GRC.',
    securityFootnote: 'Conexão criptografada · Sessão isolada por empresa',
    pillarRiscos: 'Riscos',
    pillarRiscosDesc: 'Matriz, tratamentos e aceites formais',
    pillarControles: 'Controles',
    pillarControlesDesc: 'Mapeamento, evidências e auditorias',
    pillarGapAnalysis: 'Gap Analysis',
    pillarGapAnalysisDesc: 'Frameworks, scoring e planos de ação',
    backToSite: 'Voltar ao site',
    previewNote: 'Painel ilustrativo. Os seus dados aparecem depois de entrar.',
    welcomeBack: 'Acessar a plataforma',
    signInToContinue: 'Use as credenciais da sua organização.',
    emailLabel: 'E-mail',
    passwordLabel: 'Senha',
    rememberEmail: 'Lembrar meu e-mail',
    showPassword: 'Mostrar senha',
    hidePassword: 'Ocultar senha',
  },
  register: {
    createAccount: 'Criar sua conta',
    trialBadge: '14 dias de teste grátis · sem cartão de crédito',
    selectedPlan: 'Plano selecionado',
    fullName: 'Nome completo *',
    fullNamePlaceholder: 'Seu nome completo',
    email: 'E-mail *',
    password: 'Senha *',
    passwordPlaceholder: 'Mínimo 8 caracteres',
    confirmPassword: 'Confirmar senha *',
    confirmPasswordPlaceholder: 'Repita sua senha',
    companyName: 'Nome da empresa *',
    companyNamePlaceholder: 'Nome da sua empresa',
    cnpj: 'CNPJ',
    cnpjOptional: '(opcional)',
    termsAccept: 'Li e aceito os',
    termsOfUse: 'Termos de Uso',
    and: 'e a',
    privacyPolicy: 'Política de Privacidade',
    createFreeAccount: 'Criar conta grátis',
    createAndSubscribe: 'Criar conta e assinar',
    creatingAccount: 'Criando conta...',
    alreadyHaveAccount: 'Já tem uma conta?',
    signIn: 'Fazer login',
    backToSite: 'Voltar ao site',
    accountCreated: 'Conta criada com sucesso!',
    errorEmailAlreadyRegistered: 'Este email já está cadastrado. Faça login.',
    errorGeneric: 'Erro ao criar conta',
    strengthWeak: 'Fraca',
    strengthFair: 'Regular',
    strengthMedium: 'Média',
    strengthStrong: 'Forte',
    strengthVeryStrong: 'Muito forte',
    checkMinChars: 'Mínimo 8 caracteres',
    checkUppercase: 'Letra maiúscula',
    checkNumber: 'Número',
    checkSpecial: 'Caractere especial',
    validationNameMin: 'Nome deve ter no mínimo 2 caracteres',
    validationEmailRequired: 'Email é obrigatório',
    validationEmailInvalid: 'Email inválido',
    validationPasswordMin: 'Senha deve ter no mínimo 8 caracteres',
    validationConfirmPassword: 'Confirme sua senha',
    validationPasswordsMatch: 'As senhas não coincidem',
    validationCompanyRequired: 'Nome da empresa é obrigatório',
    validationTermsRequired: 'Você deve aceitar os Termos de Uso',
  },
  forgotPassword: {
    title: 'Recuperar acesso',
    description: 'Informe o e-mail da sua organização. Se houver uma conta correspondente, enviaremos um link seguro.',
    email: 'E-mail',
    emailPlaceholder: 'seu.email@empresa.com',
    cancel: 'Cancelar',
    send: 'Enviar link de recuperação',
    sending: 'Enviando...',
    successMessage: 'Se o e-mail estiver cadastrado, você receberá as instruções de recuperação. Verifique também a pasta de spam/lixo eletrônico.',
    validationEmailRequired: 'Email é obrigatório',
    validationEmailInvalid: 'Email inválido',
    eyebrow: 'Acesso à conta',
    sentEyebrow: 'Solicitação recebida',
    sentTitle: 'Verifique seu e-mail',
    sentTo: 'Solicitação enviada para',
    backToLogin: 'Voltar ao login',
    useAnotherEmail: 'Usar outro e-mail',
    rateLimited: 'Muitas solicitações foram feitas. Aguarde uma hora antes de tentar novamente.',
    unavailable: 'Não foi possível solicitar o link agora. Aguarde um momento e tente novamente.',
  },
  // Política de senha — namespace próprio porque os três ecrãs que definem
  // senha partilham a MESMA regra; antes cada um tinha as suas chaves e as
  // suas regras, e só um deles exigia maiúscula, minúscula e número.
  // O dossiê do ROPA — a vista de leitura que o módulo não tinha.
  // A lista de ROPAs — o nível que faltava. A aba mostrava os TRATAMENTOS como
  // se fossem os ROPA, e o contentor (`ropa_exercicios`) vivia noutra aba sem
  // ligação nenhuma a esta, com zero processos associados.
  ropaLista: {
    subtitulo: 'Cada ROPA agrupa os tratamentos de um sistema, projeto ou levantamento — {lei}.',
    buscar: 'Buscar ROPA...',
    colRopa: 'ROPA',
    colVersao: 'Versão',
    colTratamentos: 'Tratamentos',
    semDescricao: 'Sem descrição',
    novoRopa: 'Novo ROPA',
    novoTratamento: 'Novo tratamento',
    voltarRopas: 'Voltar aos ROPAs',
    voltarTratamentos: 'Voltar a {ropa}',
    emptyTitulo: 'Nenhum ROPA registrado',
    emptyDescricao: 'Crie um ROPA para o sistema ou projeto que vai mapear e importe a planilha para dentro dele.',
    semTratamentosTitulo: 'Nenhum tratamento neste ROPA',
    semTratamentosDescricao: 'Crie o primeiro tratamento ou importe a planilha do levantamento.',
    // Nada fica invisível: os tratamentos importados sem ROPA têm uma linha.
    semRopaNome: 'Tratamentos sem ROPA',
    semRopaDescricao: 'Importados ou criados sem ROPA associado. Edite cada um para o arrumar.',
  },
  ropaDossie: {
    semArea: 'Área não definida',
    risco: 'Risco {nivel}',
    riscoPorClassificar: 'Risco por classificar',
    pessoaDesconhecida: 'Utilizador removido',
    transferenciaInternacional: 'Transferência internacional',
    decisaoAutomatizada: 'Decisão automatizada',
    percursoTitulo: 'Percurso do dado',
    percursoNota: 'Da origem ao descarte. Abra uma etapa para ler o registro.',
    verTodasEtapas: 'Ver todas as etapas',
    recolherEtapas: 'Recolher etapas',
    porPreencher: 'Por preencher',
    etapaSemTexto: 'Este processo não descreve esta etapa.',
    basesTitulo: 'Base legal',
    baseUnica: 'Uma base legal legitima este tratamento.',
    basesVarias: '{total} bases legais, cada uma legitimando uma parte do tratamento.',
    basePorClassificar: 'Por classificar',
    voltarLista: 'Voltar à lista',
    verDossie: 'Ver dossiê',
    anterior: 'Anterior',
    proximo: 'Próximo',
    posicao: '{atual} de {total}',
  },
  creditosIA: {
    esgotadosTitulo: 'Créditos de IA esgotados',
    aCabarTitulo: 'Restam {n} créditos de IA',
    aCabarDescricao: 'Já usou a maior parte da franquia deste mês: {n} de {total} por gastar.',
    gerirCreditos: 'Gerir créditos',
    verPlano: 'Ver o meu plano',
    dispensar: 'Agora não',
    usado: 'Gastou 1 crédito de IA',
    usadoCom: '1 crédito de IA · {funcionalidade}',
    restam: 'Restam {n} de {total}',
    esgotadosAdmin: 'Os assistentes inteligentes estão indisponíveis até que mais créditos sejam liberados.',
    esgotadosUtilizador: 'Para continuar usando os assistentes inteligentes, entre em contato com o administrador da sua conta.',
  },
  erroInesperado: {
    titulo: 'Algo deu errado',
    corpo: 'Ocorreu um erro inesperado. Tente novamente ou recarregue a página.',
    tentarNovamente: 'Tentar novamente',
    recarregar: 'Recarregar página',
    detalhes: 'Detalhes do erro (desenvolvimento)',
  },
  protectedRoute: {
    deniedTitle: 'Acesso negado',
    /* Faltar permissão e não ter comprado o módulo pedem respostas
       diferentes: o administrador da empresa não concede o que não está
       no plano. */
    planTitle: 'Fora do seu plano',
    planBody: 'Este módulo não faz parte do plano contratado pela sua empresa.',
    planHint: 'Fale com quem contratou o Akuris para incluir este módulo.',
    deniedBody: 'Você não tem permissão para acessar este módulo ou realizar esta ação.',
    deniedHint: 'Entre em contato com o administrador para solicitar acesso.',
    /* Falhar a LER a permissão não é o mesmo que não a ter. Sem estas
       três linhas, uma rede em baixo dizia a um super admin que ele não
       tinha permissão e o mandava abrir um pedido de acesso. */
    indisponivelTitle: 'Não foi possível confirmar suas permissões',
    indisponivelBody: 'A verificação de acesso não respondeu. Isso não significa que você perdeu o acesso — apenas que não conseguimos confirmá-lo agora.',
    indisponivelHint: 'Tente novamente em instantes. Se persistir, é um problema de conexão, não de permissão.',
    tentarNovamente: 'Tentar novamente',
    backToDashboard: 'Voltar ao início',
  },
  politicaSenha: {
    requirements: 'Requisitos da senha:',
    minChars: 'Mínimo 8 caracteres',
    uppercase: 'Uma letra maiúscula',
    lowercase: 'Uma letra minúscula',
    number: 'Um número',
    resumo: 'A senha precisa de 8 caracteres, com maiúscula, minúscula e número.',
  },
  notifications: {
    sendFailed: 'A ação foi gravada, mas não foi possível avisar o destinatário.',
    sendFailedMany: 'A ação foi gravada, mas {total} aviso(s) não foram entregues.',
    openRecord: 'Abrir registro',
    recordMissing: 'O registro referido já não existe.',
    viewDetails: 'Ver detalhes',
    title: 'Notificações',
    eyebrow: 'NOTIFICAÇÕES',
    markAll: 'Marcar todas',
    markAllRead: 'Marcar todas como lidas',
    unread: 'não lidas',
    unreadOne: 'não lida',
    loginToView: 'Faça login para ver notificações',
    noNotifications: 'Nenhuma notificação',
    allCaughtUp: 'Tudo em dia',
    summaryUrgentOne: '1 urgente',
    summaryUrgent: '{n} urgentes',
    summaryOpen: '{n} em aberto',
    allCaughtUpDesc: 'Você não tem notificações pendentes no momento.',
    urgent: 'Urgente',
    attention: 'Atenção',
    success: 'Sucesso',
    info: 'Info',
    errorMarkRead: 'Não foi possível marcar a notificação como lida',
    more: 'Mais',
    moreModules: 'Mais módulos',
    viewAll: 'Ver todas',
    groupUrgent: 'Urgentes',
    groupAttention: 'Atenção',
    groupInfo: 'Informativo',
    justNow: 'agora',
    modules: {
      riscos: 'Riscos',
      documentos: 'Documentos',
      contratos: 'Contratos',
      controles: 'Controles',
      incidentes: 'Incidentes',
      ativos: 'Ativos',
      aprovacoes: 'Aprovações',
      gap: 'Gap Analysis',
      duediligence: 'Due Diligence',
      denuncias: 'Denúncias',
      sistema: 'Sistema',
    },
  },
  dashboard: {
    criticalAlertsTooltip: 'Riscos críticos + não conformidades críticas + incidentes críticos + prazos vencidos',
    criticalAlertsBasis: 'Severidade residual',
    riscosCriticosBasis: 'residual',
    criticalAlertsRisks: 'Riscos críticos',
    criticalAlertsGaps: 'Não conformidades críticas',
    criticalAlertsIncidents: 'Incidentes críticos',
    criticalAlertsOverdue: 'Prazos vencidos',
    title: 'Dashboard Executivo',
    commandCenter: 'Centro de Comando',
    hello: 'Olá',
    criticalAlerts: 'Alertas Críticos',
    activeControls: 'Controles Ativos',
    compliance: 'Conformidade',
    maturity: 'Maturidade GRC',
    maturityDesc: 'Visão consolidada de todos os módulos de governança, risco e compliance',
    upcomingExpirations: 'Próximos Vencimentos',
    noExpirations: 'Nenhum vencimento nos próximos 60 dias',
    recentActivities: 'Atividades Recentes',
    noActivities: 'Nenhuma atividade recente encontrada',
    riskEvolution: 'Evolução dos Riscos',
    weekly: 'Semanal',
    monthly: 'Mensal',
    yearly: 'Anual',
    critical: 'Críticos',
    high: 'Altos',
    medium: 'Médios',
    low: 'Baixos',
    noCritical: 'Sem Críticos',
    criticalRisks: 'Riscos Críticos',
    period: 'Período',
    summary: 'Resumo',
    error: 'Erro',
    tryAgain: 'Tentar novamente',
    positiveHighlights: 'Destaques Positivos',
    alerts: 'Alertas',
    prioritizedRecommendations: 'Recomendações Priorizadas',
    highPriority: 'Alta',
    mediumPriority: 'Média',
    lowPriority: 'Baixa',
    healthScore: 'Saúde Organizacional',
    excellent: 'Excelente',
    good: 'Bom',
    warning: 'Atenção',
    criticalStatus: 'Crítico',
    noData: 'Sem dados',
  },
  /*
    Os quatro grupos são as quatro parcelas de `criticalBreakdown`. Saíram
    "Riscos Altos", "Denúncias Pendentes" e "Controles Vencendo": nenhum dos
    três somava para o número que o diálogo diz explicar.
  */
  alertsDialog: {
    title: 'Detalhamento de Alertas Críticos',
    criticalRisks: 'Riscos críticos',
    criticalGaps: 'Não conformidades críticas',
    criticalIncidents: 'Incidentes críticos',
    overdueDeadlines: 'Prazos vencidos',
    risksShort: 'Riscos',
    gapsShort: 'Não conformidades',
    incidentsShort: 'Incidentes',
    deadlinesShort: 'Prazos',
    viewAll: 'Ver todos',
    noAlerts: 'Nenhum alerta crítico no momento.',
    allGood: 'Todos os indicadores estão dentro do esperado.',
    additionalRisks: 'riscos adicionais',
    additionalGaps: 'não conformidades adicionais',
    additionalIncidents: 'incidentes adicionais',
    additionalDeadlines: 'prazos adicionais',
  },
  kpi: {
    /*
      Singular e plural, decididos pelo número.

      Os rótulos abaixo (`incidentsOpen` e companhia) são string fixa no
      plural, e por isso a linha de contexto escrevia "1 Incidentes abertos".
      Quem conta usa `rotulo`; os antigos ficam para os títulos de secção,
      onde não há número à frente.
    */
    rotulo: {
      risks: { one: 'risco', other: 'riscos' },
      actionPlansOpen: { one: 'plano em aberto', other: 'planos em aberto' },
      incidentsOpen: { one: 'incidente aberto', other: 'incidentes abertos' },
      assets: { one: 'ativo', other: 'ativos' },
      documents: { one: 'documento', other: 'documentos' },
      contractsActive: { one: 'contrato ativo', other: 'contratos ativos' },
      dueDiligenceActive: { one: 'due diligence ativa', other: 'due diligence ativas' },
      reportsOpen: { one: 'denúncia aberta', other: 'denúncias abertas' },
    },
    /**
     * O número que dá referência à contagem: "11 riscos, 1 crítico".
     *
     * Há variantes femininas porque o qualificador concorda com o substantivo
     * que qualifica: contratos ficam "vencidos", avaliações ficam "vencidas".
     */
    qualificador: {
      critical: { one: '{count} crítico', other: '{count} críticos' },
      high: { one: '{count} alto', other: '{count} altos' },
      overdue: { one: '{count} atrasado', other: '{count} atrasados' },
      expired: { one: '{count} vencido', other: '{count} vencidos' },
      expiredFem: { one: '{count} vencida', other: '{count} vencidas' },
      expiring: '{count} vencendo',
      pending: { one: '{count} pendente', other: '{count} pendentes' },
      new: { one: '{count} nova', other: '{count} novas' },
      month: '{count} no mês',
    },
    incidentsOpen: 'Incidentes abertos',
    actionPlansOpen: 'Planos em aberto',
    contractsActive: 'Contratos ativos',
    dueDiligenceActive: 'Due Diligence ativas',
    reportsOpen: 'Denúncias abertas',
    hint: {
      assets: 'Total de ativos registrados',
      risks: 'Total de riscos registrados',
      incidentsOpen: 'Incidentes abertos ou em investigação',
      actionPlansOpen: 'Planos de ação por concluir',
      contractsActive: 'Contratos com estado ativo',
      documents: 'Total de documentos ({active} ativos)',
      dueDiligenceActive: 'Avaliações de due diligence em curso',
      reportsOpen: 'Denúncias novas ou em andamento',
    },
    assets: 'Ativos',
    alerts: 'Alertas',
    controls: 'Controles',
    incidents: 'Incidentes',
    contracts: 'Contratos',
    documents: 'Documentos',
    compliance: 'Conformidade',
    expiring: 'vencendo',
    expired: 'vencidos',
    month: 'mês',
    pending: 'pendentes',
    frameworks: 'frameworks',
    risks: 'Riscos',
    critical: 'críticos',
    high: 'altos',
    actionPlans: 'Planos de Ação',
    overdue: 'atrasados',
    dueDiligence: 'Due Diligence',
    reports: 'Denúncias',
    new: 'novas',
  },
  expirations: {
    contract: 'Contrato',
    license: 'Licença',
    key: 'Chave',
    control: 'Controle',
  },
  activities: {
    newRisk: 'Novo risco identificado',
    newControl: 'Novo controle implementado',
    documentAdded: 'Documento adicionado',
    newAudit: 'Nova auditoria iniciada',
    newComplaint: 'Nova denúncia recebida',
    recordUpdated: 'Registro atualizado',
    campo: {
      nivel_risco_residual: 'Severidade residual',
      nivel_risco_inicial: 'Severidade inerente',
      status: 'Status',
      criticidade: 'Criticidade',
      responsavel_id: 'Responsável',
      proxima_avaliacao: 'Próxima avaliação',
      prazo: 'Prazo',
      codigo: 'Código',
      nome: 'Nome',
      descricao: 'Descrição',
      tipo: 'Tipo',
      categoria: 'Categoria',
      frequencia: 'Frequência',
    },
    today: 'Hoje',
    yesterday: 'Ontem',
    fieldChanged: '{campo}: {de} → {para}',
    fieldSet: '{campo} definido: {valor}',
    recordDeleted: 'Registro removido',
  },
  modules: {
    // Riscos
    riscos: {
      title: 'Gestão de Riscos',
      description: 'Identifique, avalie e monitore riscos organizacionais de forma estruturada',
      newRisk: 'Novo Risco',
      matrix: 'Matriz',
      categories: 'Categorias',
      total: 'Total de Riscos',
      critical: 'Críticos',
      high: 'Altos',
      treated: 'Tratados',
      pendingApproval: 'Pendentes Aprovação',
      accepted: 'Aceitos',
      filterByStatus: 'Filtrar por status',
      filterByLevel: 'Filtrar por nível',
      acceptance: 'Aceite',
      treatments: 'Tratamentos',
      history: 'Histórico',
      auditTrail: 'Trilha de Auditoria',
      exportPDF: 'Exportar PDF',
      exportCSV: 'Exportar CSV',
    },
    // Incidentes
    incidentes: {
      title: 'Incidentes',
      description: 'Gerencie incidentes de segurança e acompanhe tratamentos',
      newIncident: 'Novo Incidente',
      total: 'Total',
      open: 'Abertos',
      inProgress: 'Em Andamento',
      resolved: 'Resolvidos',
      criticalIncidents: 'Críticos',
      treatments: 'Tratamentos',
      communications: 'Comunicações',
      evidences: 'Evidências',
    },
    // Ativos
    ativos: {
      title: 'Gestão de Ativos',
      description: 'Gerencie todos os ativos da organização de forma centralizada',
      newAsset: 'Novo Ativo',
      import: 'Importar',
      total: 'Total de Ativos',
      active: 'Ativos',
      criticalAssets: 'Críticos',
      maintenance: 'Em Manutenção',
      maintenances: 'Manutenções',
      auditTrail: 'Trilha de Auditoria',
    },
    // Licenças
    licencas: {
      title: 'Licenças de Software',
      description: 'Gerencie e monitore licenças e suas renovações',
      newLicense: 'Nova Licença',
      total: 'Total de Licenças',
      totalDesc: 'Licenças registradas',
      active: 'Licenças Ativas',
      activeDesc: 'Em vigor',
      expiring: 'A Vencer',
      expiringDesc: 'Próximos 30 dias',
      expired: 'Vencidas',
      expiredDesc: 'Expiradas',
    },
    // Chaves
    chaves: {
      title: 'Chaves Criptográficas',
      description: 'Gerencie e monitore chaves criptográficas e rotações',
      newKey: 'Nova Chave',
      total: 'Total de Chaves',
      active: 'Ativas',
      rotating: 'Em Rotação',
      expired: 'Vencidas',
    },
    // Documentos
    documentos: {
      title: 'Documentos',
      description: 'Gerencie documentos, políticas e procedimentos da empresa de forma centralizada',
      newDocument: 'Novo Documento',
      categories: 'Categorias',
      uploadMultiple: 'Upload em Lote',
      total: 'Total',
      published: 'Publicados',
      pendingApproval: 'Pendentes Aprovação',
      expiringSoon: 'Vencendo',
    },
    // Contratos
    contratos: {
      title: 'Contratos',
      description: 'Gerencie contratos, fornecedores e acompanhe vencimentos de forma centralizada',
      newContract: 'Novo Contrato',
      suppliers: 'Fornecedores',
      templates: 'Templates',
      total: 'Total',
      active: 'Ativos',
      expiring: 'Vencendo',
      expired: 'Vencidos',
    },
    // Continuidade
    continuidade: {
      title: 'Continuidade de Negócios',
      description: 'Gerencie planos de continuidade, tarefas e testes',
      newPlan: 'Novo Plano',
      total: 'Total de Planos',
      active: 'Ativos',
      tested: 'Testados',
      revision: 'Em Revisão',
    },
    // Planos de Ação
    planosAcao: {
      title: 'Planos de Ação',
      description: 'Visão consolidada de todas as suas pendências e ações',
      newPlan: 'Novo Plano',
      myItems: 'Meus Itens',
      allPlans: 'Todos os Planos',
      pending: 'Pendentes',
      inProgress: 'Em Andamento',
      completed: 'Concluídos',
      delayed: 'Atrasados',
    },
    // Políticas
    politicas: {
      title: 'Políticas e Treinamentos',
      description: 'Gerencie políticas corporativas, aceites e questionários de validação',
      newPolicy: 'Nova Política',
      total: 'Total',
      published: 'Publicadas',
      acceptances: 'Aceites',
      pending: 'Pendentes',
    },
    // Governança / Controles
    governanca: {
      title: 'Governança',
      controlsTitle: 'Controles Internos',
      controlsDesc: 'Gerencie os controles internos da organização',
      auditsTitle: 'Auditorias',
      auditsDesc: 'Gerencie auditorias e acompanhe evidências',
      controls: 'Controles',
      audits: 'Auditorias',
      newControl: 'Novo Controle',
      newAudit: 'Nova Auditoria',
    },
    // Sistemas
    sistemas: {
      title: 'Sistemas',
      description: 'Gerencie os sistemas da organização',
      newSystem: 'Novo Sistema',
    },
    // Contas Privilegiadas
    contasPrivilegiadas: {
      title: 'Contas Privilegiadas',
      description: 'Gerencie contas com acessos privilegiados aos sistemas',
      newAccount: 'Nova Conta',
      total: 'Total',
      active: 'Ativas',
      expiring: 'Vencendo',
      revoked: 'Revogadas',
    },
    // Revisão de Acessos
    revisaoAcessos: {
      title: 'Revisão de Acessos',
      description: 'Revise usuários dos sistemas e contas privilegiadas em uma mesma campanha',
      newReview: 'Nova Revisão',
      total: 'Total',
      inProgress: 'Em Andamento',
      completed: 'Concluídas',
      delayed: 'Atrasadas',
    },
    // Due Diligence
    dueDiligence: {
      title: 'Due Diligence',
      description: 'Gerencie avaliações digitais de fornecedores com questionários personalizados e scoring automático',
      // Nome de aba é substantivo, como «Fornecedores» e «Templates» ao lado.
      // «Nova Avaliação» era uma acção usada como rótulo de secção.
      newAssessment: 'Avaliações',
      suppliers: 'Fornecedores',
      templates: 'Templates',
      questions: 'Perguntas',
      reports: 'Relatórios',
    },
    // Privacidade
    privacidade: {
      title: 'Privacidade',
      description: 'Proteção de dados pessoais, mapeamento e ROPA (LGPD)',
      newRecord: 'Novo Registro',
      ropa: 'ROPA',
      mapping: 'Mapeamento',
      personalData: 'Dados Pessoais',
      requests: 'Solicitações',
    },
    // Denúncia
    denuncia: {
      title: 'Canal de Denúncia',
      description: 'Gerencie denúncias e mantenha um ambiente ético e transparente',
      newComplaint: 'Nova Denúncia',
      total: 'Total',
      new: 'Novas',
      inProgress: 'Em Andamento',
      resolved: 'Resolvidas',
      categories: 'Categorias',
      reports: 'Relatórios',
      settings: 'Configurações',
    },
    // Relatórios
    relatorios: {
      title: 'Relatórios',
      description: 'Crie e gerencie relatórios customizáveis para auditorias, diretoria e órgãos reguladores',
      newReport: 'Novo Relatório',
      generate: 'Gerar',
      templates: 'Templates',
      total: 'Total',
      generated: 'Gerados',
      scheduled: 'Agendados',
    },
    // Gap Analysis
    gapAnalysis: {
      title: 'Gap Analysis',
      description: 'Avalie a aderência da organização aos principais frameworks de mercado',
      frameworks: 'Frameworks',
      newFramework: 'Adicionar Framework',
      assessment: 'Avaliação',
      requirements: 'Requisitos',
      remediation: 'Remediação',
      soa: 'Declaração de Aplicabilidade',
    },
  },
  statusLabels: {
    // Tipos de Ativos/Sistemas
    aplicacao: 'Aplicação',
    banco_dados: 'Banco de Dados',
    sistema_operacional: 'Sistema Operacional',
    hardware: 'Hardware',
    software: 'Software',
    rede: 'Rede',
    nuvem: 'Nuvem',
    servidor: 'Servidor',
    dispositivo: 'Dispositivo',
    seguranca: 'Segurança',
    comunicacao: 'Comunicação',
    // Documentos
    politica: 'Política',
    procedimento: 'Procedimento',
    instrucao: 'Instrução',
    formulario: 'Formulário',
    relatorio: 'Relatório',
    certificado: 'Certificado',
    contrato: 'Contrato',
    documento: 'Documento',
    manual: 'Manual',
    norma: 'Norma',
    registro: 'Registro',
    // Classificações
    publica: 'Pública',
    interna: 'Interna',
    restrita: 'Restrita',
    confidencial: 'Confidencial',
    // Criticidade
    critico: 'Crítico',
    critica: 'Crítica',
    alto: 'Alto',
    alta: 'Alta',
    medio: 'Médio',
    media: 'Média',
    baixo: 'Baixo',
    baixa: 'Baixa',
    muito_alto: 'Muito Alto',
    muito_baixo: 'Muito Baixo',
    // Status workflow
    pendente: 'Pendente',
    pendente_aprovacao: 'Pendente Aprovação',
    em_andamento: 'Em Andamento',
    em_analise: 'Em Análise',
    em_revisao: 'Em Revisão',
    em_investigacao: 'Em Investigação',
    concluido: 'Concluído',
    concluida: 'Concluída',
    cancelado: 'Cancelado',
    cancelada: 'Cancelada',
    aprovado: 'Aprovado',
    aprovada: 'Aprovada',
    rejeitado: 'Rejeitado',
    rejeitada: 'Rejeitada',
    nao_aplicavel: 'Não Aplicável',
    arquivado: 'Arquivado',
    arquivada: 'Arquivada',
    rascunho: 'Rascunho',
    planejamento: 'Planejamento',
    contido: 'Contido',
    resolvido: 'Resolvido',
    resolvida: 'Resolvida',
    atendida: 'Atendida',
    fechado: 'Fechado',
    aberto: 'Aberto',
    nova: 'Nova',
    novo: 'Novo',
    // Auditoria
    ti: 'TI',
    compliance: 'Compliance',
    operacional: 'Operacional',
    externa: 'Externa',
    financeira: 'Financeira',
    // Controles
    preventivo: 'Preventivo',
    detectivo: 'Detectivo',
    corretivo: 'Corretivo',
    compensatorio: 'Compensatório',
    // Status item
    ativo: 'Ativo',
    ativa: 'Ativa',
    inativo: 'Inativo',
    inativa: 'Inativa',
    vencido: 'Vencido',
    vencida: 'Vencida',
    expirado: 'Expirado',
    expirada: 'Expirada',
    revogado: 'Revogado',
    revogada: 'Revogada',
    suspenso: 'Suspenso',
    suspensa: 'Suspensa',
    a_vencer: 'A Vencer',
    em_renovacao: 'Em Renovação',
    em_rotacao: 'Em Rotação',
    descontinuado: 'Descontinuado',
    // Riscos
    identificado: 'Identificado',
    analisado: 'Analisado',
    tratado: 'Tratado',
    monitorado: 'Monitorado',
    aceito: 'Aceito',
    mitigado: 'Mitigado',
    mitigar: 'Mitigar',
    transferir: 'Transferir',
    aceitar: 'Aceitar',
    evitar: 'Evitar',
    // Frequências
    diaria: 'Diária',
    diario: 'Diário',
    semanal: 'Semanal',
    quinzenal: 'Quinzenal',
    mensal: 'Mensal',
    bimestral: 'Bimestral',
    trimestral: 'Trimestral',
    semestral: 'Semestral',
    anual: 'Anual',
    sob_demanda: 'Sob Demanda',
    // Conformidade
    conforme: 'Conforme',
    nao_conforme: 'Não Conforme',
    parcial: 'Parcial',
    parcialmente_conforme: 'Parcialmente Conforme',
  },
} as const;
